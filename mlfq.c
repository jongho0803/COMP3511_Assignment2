...
//Helper: Dealing with the first queue
void handle_fir(struct Queue* fir, struct Queue* sec, struct GanttChartItem chart[MAX_GANTT_CHART], int* chart_count, int duration) {
    
    int now = queue_peek(fir);
    gantt_chart_update(chart, chart_count, process_table[now].name, duration);
    process_table[now].burst_time--;
    process_table[now].remain_time++;

    if (process_table[now].burst_time == 0)
        queue_dequeue(fir);
    else if (process_table[now].remain_time == tq0) {
        queue_dequeue(fir);                                             
        queue_enqueue(sec, now);
    }  
    return;
}

//Helper: Dealing with the second queue
void handle_sec(struct Queue* sec, struct Queue* last, struct GanttChartItem chart[MAX_GANTT_CHART], int* chart_count, int duration) {
    
    int now = queue_peek(sec);
    gantt_chart_update(chart, chart_count, process_table[now].name, duration);
    process_table[now].burst_time--;
    process_table[now].remain_time++;
    ...
    if (process_table[now].burst_time == 0)
        queue_dequeue(sec);
    else if (process_table[now].remain_time == tq0 + tq1) {
        queue_dequeue(sec);
        queue_enqueue(last, now);
    }
    return;
}

//Helper: Dealing with the last queue
void handle_last(struct Queue* last, struct GanttChartItem chart[MAX_GANTT_CHART], int* chart_count, int duration) {
    
    int now = queue_peek(last);
    gantt_chart_update(chart, chart_count, process_table[now].name, duration);
    process_table[now].burst_time--;
...
void mlfq() {
    ...
    struct Queue fir, sec, last;                                            //make three orders of queue
    queue_init(&fir);
    queue_init(&sec);
    queue_init(&last);

    int total_time = 0;                                                     //find out total cpu run time
    for (int i = 0; i < process_table_size; i++) {
        total_time += process_table[i].burst_time;
    }

    for (int i = 0; i < total_time; i++) {                                  //compare every second
        for (int j = 0; j < process_table_size; j++) {                      //if process reaches arrival time{
            if (i == process_table[j].arrival_time) {
                queue_enqueue(&fir, j);                                     //put in first queue
            }
        }

        if (!queue_is_empty(&fir)) {
            handle_fir(&fir, &sec, chart, &sz_chart, 1);                    //work on first queue
        }
        else if (queue_is_empty(&fir) && !queue_is_empty(&sec)) {
            handle_sec(&sec, &last, chart, &sz_chart, 1);                   //work on second queue
        }
        else {
            handle_last(&last, chart, &sz_chart, 1);                        //work on last queue
        }
    }
